...
export const getSupervisors = async (firstName = "Joe", lastName = "Power") => {
  ...
  const supervisorPromises = R.map(x => fetchList(x.DEUSERID), jsonSupervisor);
...
const fetchList = async (id, firstName = "Joe", lastName = "Power") => {
  const auth = Base64.encode(
    firstName + " " + lastName + ":8D63-C786-B0CC-3C30-9D40-6ECF-9E84-3498"
  );
  const requestHeaders = {
